...
/** @file src_interactive_tests/test_visualizer_intersection_finder.cpp
 *
 *  @brief visual test rig for Makena::IntersectionFinder
 *
...
class GLBufferManager {
  ...
  private:
    ...
    std::vector<Makena::Vec3> mPointsThickLines;
    std::vector<Makena::Vec3> mColorsThickLines;
...
void GLBufferManager::resetOneStep()
{
    ...
    mPointsLines.clear();
    mColorsLines.clear();
    mPointsThickLines.clear();
    mColorsThickLines.clear();
    mPointsPoints.clear();
    mColorsPoints.clear();
}
...
void GLBufferManager::addThickLines(
    const std::vector<Makena::Vec3>& p,
    const std::vector<Makena::Vec3>& c
) {

    mPointsThickLines.insert(mPointsThickLines.begin(),   p.begin(), p.end());
    mColorsThickLines.insert(mColorsThickLines.begin(),   c.begin(), c.end());
}
...
void GLBufferManager::renderThickLines()
{
    auto arrLinesVertices = makeGLbufferArray3D( mPointsThickLines );
    auto arrLinesColors   = makeGLbufferArray3D( mColorsThickLines );
    ...
    glBufferData(
        GL_ARRAY_BUFFER,
        mPointsThickLines.size()*sizeof(GLfloat)*3,
        arrLinesVertices.get(),
        GL_DYNAMIC_DRAW
    );
    ...
    glBufferData(
        GL_ARRAY_BUFFER,
        mColorsThickLines.size()*sizeof(GLfloat)*3,
        arrLinesColors.get(),
        GL_DYNAMIC_DRAW
    );
    ...
    glDrawArrays(GL_LINES, 0, mPointsThickLines.size());
...
class TestRig : public GlfwCallable, public TwCallable {
  ...
  private:
    ...
    Makena::Manifold mManifold1;
    Makena::Manifold mManifold2;

    Makena::Quaternion mQ1;
    Makena::Quaternion mQ2;
    Makena::Vec3       mCoM1;
    Makena::Vec3       mCoM2;

    double mEpsilonZero;
    double mEpsilonAngle;
    double mEpsilonZeroPCA;
};
...
void TestRig::loadContentsFromFiles(char* filename1, char* filename2)
{
    ...
    mManifold1.findConvexHull(points1, pred1);
    mManifold2.findConvexHull(points2, pred2);
}
...
void TestRig::render(GLBufferManager& buf)
{
    ...
    std::vector<Makena::Vec3> verticesThickEdges;
    std::vector<Makena::Vec3> colorsThickEdges;
    ...
    auto Qmat1 = mQ1.rotationMatrix();
    auto Qmat2 = mQ2.rotationMatrix();
/*
    mManifold1.makeOpenGLVerticesColorsNormalsForTriangles(
        red,
        0.2,
        verticesFaces,
        colorsFaces,
    ...
    mManifold2.makeOpenGLVerticesColorsNormalsForTriangles(
        blue,
        0.2,
        verticesFaces,
        colorsFaces,
...
*/
    ...
    mManifold1.makeOpenGLVerticesColorsForTriangleWireFrame(
        red, verticesEdges, colorsEdges, 1.0, Qmat1, mCoM1
    );

    mManifold2.makeOpenGLVerticesColorsForTriangleWireFrame(
        blue, verticesEdges, colorsEdges, 1.0, Qmat2, mCoM2
    );
                     

    Makena::IntersectionFinder finder(
        mManifold1,
        Qmat1,
        mCoM1,
        mManifold2,
    ...
    if (finder.dimension()==3) {
        ...
        for (auto fit = fPair.first; fit != fPair.second; fit++) {
            ...
            Makena::Vec3 faceColor;
            if (fatt.mPred==Makena::IF_FACE_INTERIOR) {
                faceColor = red;
            }
            else if (fatt.mPred==Makena::IF_INTERIOR_FACE) {
                faceColor = blue;
            }
            else {
                faceColor = gray;
            ...
            ch.makeOpenGLVerticesColorsNormalsForTriangles(
                fit,
                faceColor,
                1.0,
                verticesFaces,
        ...
        for (auto eit = ePair.first; eit != ePair.second; eit++) {
            ...
            switch (eatt.mPred) {
              case Makena::IF_EDGE_EDGE:
                break;
              case Makena::IF_EDGE_FACE:
                break;
              case Makena::IF_EDGE_INTERIOR:
                break;
              case Makena::IF_FACE_EDGE:
                break;
              case Makena::IF_FACE_FACE:
                break;
              case Makena::IF_FACE_INTERIOR:
                break;
              case Makena::IF_INTERIOR_EDGE:
                break;
              case Makena::IF_INTERIOR_FACE:
                break;
            } 
        }
    }
    else if (finder.dimension()==2) {
        auto numVertices = finder.size2D();
        auto fattr = finder.faceAttributes2D();
        Makena::Vec3 normal;
        if ( fattr.mPred == Makena::IF_FACE_FACE    ||
             fattr.mPred == Makena::IF_FACE_INTERIOR  ) {
            normal = (*fattr.mFit1)->nGCS(Qmat1);
        }
        else if ( fattr.mPred == Makena::IF_INTERIOR_FACE) {
            normal = (*fattr.mFit2)->nGCS(Qmat2);
        }
        else {
            normal = Makena::Vec3(0.0, 0.0, 0.0);
        }
        auto& att0 = finder.vertexAttributes2D(0);
        for (long i = 1; i < numVertices-1; i++) {
            auto& att1 = finder.vertexAttributes2D(i);
            auto& att2 = finder.vertexAttributes2D((i+1)%numVertices);
            verticesFaces.push_back(att0.mP);
            verticesFaces.push_back(att1.mP);
            verticesFaces.push_back(att2.mP);
            normalsFaces.push_back(normal);
            normalsFaces.push_back(normal);
            normalsFaces.push_back(normal);
            colorsFaces.push_back(gray);
            colorsFaces.push_back(gray);
            colorsFaces.push_back(gray);
            alphasFaces.push_back(1.0);
            alphasFaces.push_back(1.0);
            alphasFaces.push_back(1.0);
        }
    }
    else if (finder.dimension()==1) {
        auto& att1 = finder.vertexAttributes1D(0);
        auto& att2 = finder.vertexAttributes1D(1);
        verticesEdges.push_back(att1.mP);
        verticesEdges.push_back(att2.mP);
        colorsEdges.push_back(gray);
        colorsEdges.push_back(gray);
    }
    else if (finder.dimension()==0) {
        auto& att = finder.vertexAttributes0D();
        verticesPoints.push_back(att.mP);
        colorsPoints.push_back(gray);
    }
    buf.addTriangles(verticesFaces, colorsFaces, alphasFaces, normalsFaces);
    buf.addLines(verticesEdges, colorsEdges);
//    buf.addThickLines(verticesThickEdges, colorsThickEdges);
...
int main( int argc, char* argv[])
{
    ...
    if (argc != 3) {
        std::cerr << "Usage: test_visualizer_gjk_origin_finder "
                     "[test input file A] [test input file B]\n";
        exit(1);
    }
    ...
    TestRig              rig    (Makena::EPSILON_LINEAR,
                                 Makena::EPSILON_LINEAR,
                                 Makena::EPSILON_LINEAR );
